...
class FourDVar:
    ...
    def __init__(
        ...
        self.H_obs = np.load("indices_Central.npy")
        ...
        print(self.N_obs)
        # self.H_obs = tf.reshape(self.H_obs, (1,self.N_obs))
        ...
        self.x_bar = np.load("EOF/mean_norm.npy")
        self.S = np.load("EOF/S_y4.npy")[: self.k]
        # self.S = np.concatenate([self.S,  np.zeros(8871 - 720)])
        self.EOF = np.load("EOF/EOFs_y8_norm.npy")[:, : self.k]
        self.U = np.load("EOF/U_y4.npy")[: self.k]
        # Load mask
        ...
        loaded_s = self.model.load_weights(
            "../../transfer/Results/UNet_transfer2_relu_train2/" + "model"
        ).expect_partial()
        ...
        self.x_0 = tf.Variable(
            initial_value=np.zeros((1, self.N_z_reduced)), dtype=tf.float64
        )
        self.x_b = tf.Variable(
            initial_value=np.zeros((1, self.N_z_reduced)), dtype=tf.float64
        )
        self.y = [
            tf.Variable(initial_value=np.zeros((1, self.N_z_reduced)), dtype=tf.float64)
            for k in range(self.N_obs_per_window * self.N_cycle + 1)
        ]
    ...
    def free_run_analysis(self, x, N_forcings, length):
        ...
        for l in range(0, length):
            ...
            x2 = tf.concat(
                [tf.reshape(x, (self.N_x, self.N_y, 1)), self.forcings[N_forcings + l]],
                axis=2,
            )
            ...
            x = tf.clip_by_value(x, clip_value_min=self.min, clip_value_max=10)
    ...
    def free_run(self, x, N_forcings):
        ...
        x_t = tf.reshape(
            tf.tensor_scatter_nd_update(x_2, self.mask_expanded, x[0]),
            (self.N_x, self.N_y),
        )
        ...
        for l in range(0, self.window_length):
            ...
            x = tf.concat(
                [
                    tf.reshape(x_t, (self.N_x, self.N_y, 1)),
    ...
    def forward(self, x_t, forcings):
        ...
        for l in range(self.num_batches_per_window):
            ...
            x1 = tf.concat(
                [tf.reshape(x, (self.N_x, self.N_y, 1)), self.forcings[l]], axis=2
            )
            err = self.model(
    ...
    def loss(self, u_0):
        ...
        obs_J = 0
        x = tf.reshape(self.x_bar, (1, self.N_z_reduced)) + tf.reshape(
            self.EOF @ u, (1, self.N_z_reduced)
        )

        for k in range(
            1 + self.N * self.N_obs_per_window, (self.N + 1) * self.N_obs_per_window + 1
        ):
            ...
            x_t = self.forward(
                ...
                self.forcings[
                    ...
                    + self.num_batches_per_window
                    * (k - 1 - self.N * self.N_obs_per_window) : self.N
                    * self.window_length
                    + self.num_batches_per_window * (k - self.N * self.N_obs_per_window)
                ],
            )
            if self.obs_op == "ten":
                obs_J += self.observation_loss_ten(
                    x_t, tf.reshape(self.y[k], (1, self.N_z_reduced))
                )
            elif self.obs_op == "single":
                obs_J += self.observation_loss_single(
                    x_t, tf.reshape(self.y[k], (1, self.N_z_reduced))
                )
            else:
                obs_J += self.observation_loss(
                    x_t, tf.reshape(self.y[k], (1, self.N_z_reduced))
                )
    ...
    def run_analysis(self, cycle_num):
        ...
        for n in range(cycle_num):
            ...
            self.x_0 = tf.reshape(self.x_bar, (self.N_z_reduced)) + tf.reshape(
                self.EOF @ tf.reshape(self.result, (self.k, 1)), (self.N_z_reduced)
            )
            # Run free run and 4DVar run during one cycle
            # free = self.free_run(self.state, (self.N)*self.window_length)
            FourDRun = self.free_run(
                tf.reshape(self.x_0, (1, self.N_z_reduced)),
                (self.N) * self.window_length,
            )
            ...
            self.J_min[n] = self.J
            self.x_b = tf.reshape(
                tf.gather(tf.reshape(FourDRun[-1], self.N_z), self.unflatten, axis=0),
                (1, self.N_z_reduced),
            )
            self.state.assign(
                tf.reshape(
                    self.EOF.T
                    @ tf.reshape(self.x_b - self.x_bar, (self.N_z_reduced, 1)),
                    (1, self.k),
                )
            )
    ...
    def test(self):
        ...
        self.J_min = np.zeros(self.N_cycle)
        # Load dataset and observations object
        data = dataset(
            PATH_TO_DATA,
            self.N_obs_per_window,
            (self.window_length + 150) * self.N_cycle,
        ...
        self.x_0.assign(
            tf.reshape(
                tf.gather(
                    x_0 + np.random.normal(0, 0.0, (self.N_z)),
                    self.unflatten[0],
                    axis=0,
                ),
                (1, self.N_z_reduced),
        ...
        self.w_0 = self.EOF.T @ tf.reshape(
            self.x_0 - tf.reshape(self.x_bar, (1, self.N_z_reduced)),
            (self.N_z_reduced, 1),
        )
        ...
        climatology = tf.reshape(
            tf.cast(self.climatology[0, :, :, 0], dtype=tf.float64), (self.N_z)
        )
        self.x_b.assign(tf.reshape(self.x_bar, (1, self.N_z_reduced)))
        self.clim = self.x_b
        self.w_b = self.EOF.T @ tf.reshape(self.x_b, (self.N_z_reduced, 1))
        true_input = tf.reshape(
            tf.gather(
                tf.reshape(tf.cast(sit[0], dtype=tf.float64), (self.N_z)),
                self.unflatten[0],
                axis=0,
            ),
            (1, self.N_z_reduced),
        ...
        # Get observations
        for k in range(self.N_cycle * self.N_obs_per_window):
            obs = tf.reshape(
                tf.cast(obs_field[k], dtype=tf.float64) * self.mask.squeeze(),
                (self.N_z),
            )
            self.y[k].assign(
                tf.reshape(
                    tf.gather(obs, self.unflatten[0], axis=0), (1, self.N_z_reduced)
                )
            )
        start_time = time.time()
        ...
        np.save(self.save_pred + "J_min.npy", np.array(self.J_min))
        ...
        x_free_long = np.zeros((cycle_num * self.window_length, self.N_x, self.N_y))
        x_free_long = self.free_run_analysis(x_t_0, 0, cycle_num * self.window_length)
        np.save("long_free_cycle.npy", x_free_long)
        for i in range(cycle_num - 2):

            print(i)
            x_analysis_forecast[i] = self.free_run_analysis(
                x_4D[i, 0], i * self.window_length, 90
            )

            x_free_forecast[i] = self.free_run_analysis(
                tf.reshape(
                    tf.cast(sit[i * self.window_length], dtype=tf.float64), (self.N_z)
                ),
                i * self.window_length,
                90,
            )
        for i in range(cycle_num - 4):
            x_analysis_forecast2[i] = self.free_run_analysis(
                x_4D[i, 16], 16 + i * self.window_length, 90
            )
