...
class FourDVar:
    ...
    def cost_grad(self, x_0):
        ...
        if self.save_grad == True:
            np.save(
                self.save_pred
                + "grad/grad_{}_cycle{}.npy".format(str(self.flag_grad), str(self.N)),
                grad[0].numpy(),
            )
            self.flag_grad += 1
    ...
    def run_analysis(self, cycle_num):
        ...
        for n in range(cycle_num):
            ...
            FourDRun = self.free_run(
                tf.reshape(result.x, (1, self.N_z_reduced)),
                (self.N) * self.window_length,
            )
    ...
    def test(self):
        ...
        obs = observations(
            PATH_TO_DATA,
            self.num_batches_per_window,
            (self.window_length + 100) * self.N_cycle,
            self.std_obs,
            starting_time=self.starting_time,
        )
        ...
        self.x_0.assign(
            tf.reshape(tf.gather(x_0, self.unflatten[0], axis=0), (1, self.N_z_reduced))
        )
