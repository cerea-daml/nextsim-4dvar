...
class FourDVar():
    ...
    #### OBSERVATION LOSS ####
    @tf.function
    def observation_loss(self, x_t, y):
        Hx_t = x_t
        dy_t = 1/(self.std_obs)**2*(Hx_t - y)
        return tf.math.reduce_sum(dy_t**2, axis = -1)
    ...
    def loss(self, x_0):
        ...
        self.Jo = obs_J/2
        self.Jb = 1/self.inflation_new*tf.math.reduce_sum(dx**2, axis = -1)
        return 1/2 * J
    ...
    def run_analysis(self, cycle_num):
        ...
        for n in range(cycle_num):
            ...
            self.store_inf[n+1] = self.inflation_new    #Store the trajectory
