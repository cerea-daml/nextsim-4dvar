...
class FourDVar():
    ...
    '''
    def __init__(self, model, mask, k, N_cycle,starting_time, ftol,obs_op, time_window, inflation, std_back, std_obs, timestep, save_grad, save_pred, path_to_save, path_to_data, N_x = 128, N_y = 128):
        ...
        loaded_s = self.model.load_weights('../../transfer/Results/UNet_transfer2_relu_train2/' + "model").expect_partial()  
    ...
    def test(self):
        ...
        obs = observations(PATH_TO_DATA, self.num_batches_per_window, (self.window_length + 150)*self.N_cycle, self.std_obs, starting_time = self.starting_time)
        ...
        for i in range(cycle_num - 4):
            x_analysis_forecast2[i] = self.free_run_analysis(x_4D[i, 16], 16 + i*self.window_length, 90)
        x_free = self.free_run_analysis(x_t_0, 0, self.window_length*self.N_cycle)
        return x_4D, x_free, sit, x_analysis_forecast, x_free_forecast, x_analysis_forecast2
