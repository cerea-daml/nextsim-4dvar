...
class create_dataset_from_nextsim_outputs:
    ...
    def normalisation(self, path_to_file):
        ...
        max_input = xtrain.max(dim=["time", "lat", "lon", "x", "y"])
        print("MAX")
        print(max_input)

        climatology_before_norm = xtrain.groupby("time.dayofyear").mean("time")
        print(climatology_before_norm)
        np.save("climatology_before_norm.npy", climatology_before_norm["inputs_sit"])
...
data = create_dataset_from_nextsim_outputs(N_res=2)
data.make_sit()
