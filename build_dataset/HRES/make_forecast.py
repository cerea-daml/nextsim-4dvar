...
class create_dataset_from_nextsim_outputs : 
    ...
    def interpolate(self, time, inputs, dataset, data_name, target_shape, d, inds):
        nt = time.shape[0]
        tmp = {}
        mask = np.load('mask.npy').squeeze()*1
        mask = 1 - mask
        tmp[data_name] = []
        print(np.shape(dataset[data_name][0].values))
        for t in trange(0, nt):
            w = dataset[data_name][t]
            #w = dask.array.nan_to_num(w, 0)
            w = w.as_numpy()
            
            var1 = np.array(w).flatten()[inds]
            var2 = np.array(w).flatten()[inds]
            var = 1/2*(var1 + var2)
            var.shape = (603, 528)
            var = np.reshape(var, (603, 528))
            #var = dask.array.nan_to_num(var, 0)
            var = np.array(var)

            var = block_reduce(var[91:, 8:-8], block_size=(self.N_res, self.N_res), func=np.mean)
            tmp[data_name].append(var)
        
#           var[325] = 1/2*(var[326] + var[324])
    ...
    def create_forcings_dataset(self, nextsim_data, variable, data_name):
        ...
        liste2 = "sea-ice/forcings_2021_01_12.nc"
        dataset_val = xr.open_dataset(liste2)
        ...
        dataset_val[variable] = (
            ["time", "prec", "lat", "lon"],
            np.array(
                [
                    np.array(forcings_val[data_name])[:-4],
                np.array(forcings_val[data_name])[2:-2],
                np.array(forcings_val[data_name])[3:-1],
                np.array(forcings_val[data_name])[4:],
                ]
            ).transpose((1, 0, 2, 3)),
        )
    ...
    #                   dataset_2015,dataset_2016])
        mean = dataset_val[variable].mean(skipna=True)
 
        std = dataset_val[variable].std(skipna=True)


        dataset_val[variable] = (dataset_val[variable] - mean) / std

        print(dataset_val[variable].mean(skipna=True))
        print("WRITE")
        dataset_val.to_netcdf(path="./" + variable + "_2021_01_12_forcings.nc", mode="w")

    def create_forcings(self, path_to_data):
        self.create_forcings_dataset(path_to_data, "t2m", "var167")
        self.create_forcings_dataset(path_to_data, "u10", "var165")
        self.create_forcings_dataset(path_to_data, "v10", "var166")
#        self.create_sit_dataset(path_to_data, "sit", "sit")
    ...
    def merge_sit_forcings(self):
        ...
        x1 = x1.isel(time=slice(8, 8 + 37))
        print(x1)
        x2 = xr.open_dataset('u10_2020_11_01_forcings.nc')
        x3 = xr.open_dataset('v10_2020_11_01_forcings.nc')
        x5 = xr.open_dataset('t2m_2020_11_01_forcings.nc')
        x1["time"] = x2["time"]
        x = xr.merge([x1,x2,x3,x5])
        print(x)
        x.to_netcdf('ds_2020_11_01_input.nc', mode = 'w')


data = create_dataset_from_nextsim_outputs(N_res = 1)
#data.make_sit()
#data.create_sit_dataset("../Data/Moorings_2018m08.nc", "sithick", "sithick")
