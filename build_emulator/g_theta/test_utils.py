...
class Test:
    ...
    def FS(self, x_truth):
        ...
        for t in range(1, self.k):
            ...
            #x_pred[t] = x_pred[t]* self.mask.reshape((self.N_x, self.N_y, 1))
            #x_pred[t] = np.clip(x_pred[t], a_min = None, a_max = 12) 
            x_pred[t] = err* self.mask.reshape((self.N_x, self.N_y, 1))
            #x_pred[t] = np.clip(x_pred[t], a_min = None, a_max = 12)
            x_inputs[t] = np.concatenate(
                    (
                    err.reshape((self.N_x, self.N_y, 1)),
    ...
    def test_model(self):
        ...
        TEST_FILENAMES = [
                self.path_data + "val.tfrecords.000",
                self.path_data + "val.tfrecords.001",
                self.path_data + "val.tfrecords.002",
                self.path_data + "val.tfrecords.003",
                self.path_data + "val.tfrecords.004",
                self.path_data + "val.tfrecords.005",
                self.path_data + "val.tfrecords.006",
                self.path_data + "val.tfrecords.007",
                self.path_data + "val.tfrecords.008",
                self.path_data + "val.tfrecords.009",
                self.path_data + "val.tfrecords.010",
                self.path_data + "val.tfrecords.011",
                self.path_data + "val.tfrecords.012",
                self.path_data + "val.tfrecords.013",
                self.path_data + "val.tfrecords.014",
                self.path_data + "val.tfrecords.015",
                self.path_data + "val.tfrecords.016",
                self.path_data + "val.tfrecords.017",
                self.path_data + "val.tfrecords.018",
                self.path_data + "val.tfrecords.019",
                self.path_data + "val.tfrecords.020",
                self.path_data + "val.tfrecords.021",
                self.path_data + "val.tfrecords.022",
                self.path_data + "val.tfrecords.023"]
        fs = np.zeros((self.N_cycle, self.k))

        bias = np.zeros((self.N_cycle, self.k))
        fs_pers = np.zeros((self.N_cycle, self.k))
        x_pred = np.zeros((self.N_cycle, self.k, 128, 128, 1))
        truth =  np.zeros((self.N_cycle, self.k, 128, 128, 1))
        i = 0
        ...
        for i in trange(self.N_cycle):
            ...
            if self.save_pred == True :
                ...
                np.save(self.path+'clip_cycle_'+str(self.N_cycle) +'_truth_'+str(i)+'.npy', tru)
            #if self.save_pred == True :
                np.save(self.path +'clip_cycle_'+str(self.N_cycle) +'_pred_'+str(i)+'.npy', xpred)
