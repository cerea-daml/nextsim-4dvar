...
class PConv(layers.Conv2D):
    ...
    def __init__(self,mask,over_compensation,*args, n_channels=2, mono=False, **kwargs):
        ...
        self.mask = tf.cast(mask,'float64', name='mask')
...
class UNet4_simple(tf.keras.Model):
    '''Build a UNet with any skip connections
        Input : - mask : initial mask the partial data
                - kernel_size : int, size of the kernel for the partial convolution, except the last one
                - activation : string, activation function, as known by keras for Conv2D
    ...
    def __init__(self,mask,activation_final_layer, input_shape = (288, 256, 10), N_output = 2, over_compensation = True, kernel_size=3,activation='relu',SE_prob = 0.3,depth = 3, n_filter = 4, dropout_prob=0.2):
        super(UNet4_simple,self).__init__()
...
class UNet4(tf.keras.Model):
    ...
    def __init__(self,mask,activation_final_layer, input_shape = (288, 256, 10), N_output = 2, over_compensation = True, kernel_size=3,activation='relu',SE_prob = 0.3,depth = 3, n_filter = 4, dropout_prob=0.2):
        ...
        outputs1 = PConv(filters = 16,
                        ...
                        activation = activation,
                        use_bias=False,
                        kernel_initializer = kernel_initializer)(outputs1)
...
class UNet3(tf.keras.Model):
    ...
    def call(self, inputs_img):
        return self.model(inputs_img)
class UNet_transfer(tf.keras.Model):
    '''Build a UNet with any skip connections
        Input : - mask : initial mask the partial data
                - kernel_size : int, size of the kernel for the partial convolution, except the last one
                - activation : string, activation function, as known by keras for Conv2D
    ...
    def __init__(self,mask,train, final, N_output = 1,input_shape = (288, 256, 10),SE_prob = 0.3, kernel_size=3,activation='relu',depth = 3, n_filter = 4, dropout_prob=0.2):
        ...
        base_model = UNet3(mask=mask, input_shape = input_shape,
                            kernel_size = kernel_size,
                            activation = activation,
                            depth = 3,
                            SE_prob = SE_prob,
                            ...
                            dropout_prob = 0)
        #base_model = UNet3()
        loaded_s = base_model.load_weights('Results/UNet/' + "model")
        base_model.trainable = train
        outputs1 = base_model(inputs_img, training=False)
        ...
        #Denormalize output
        outputs1 = outputs1 * self.std_output + self.mean_output

        X = inputs_img[:,:,:,0]
        X = tf.reshape(X, (-1, 128, 128, 1))

        #Denormalize input
        X = X * self.std_input + self.mean_input
        ...
        #Renormalize
        outputs1 = (outputs1 - self.mean_input) / self.std_input
        mask2 = tf.expand_dims(mask, axis = 0)
        outputs1 = PConv(filters = 1,
                    mask=mask2,
                    over_compensation = True,
                    kernel_size = 1,
                    ...
                    activation = final,
                    use_bias=True,
                    kernel_initializer = tf.keras.initializers.Zeros())(outputs1)

        outputs1 = min_sit + outputs1
        #outputs1 = outputs1 * mask
        self.model = tf.keras.Model(inputs = inputs_img, outputs = outputs1)
        self.model.summary()
    def call(self, inputs_img):
        return self.model(inputs_img)
...
class UNet_transfer2(tf.keras.Model):
    '''Build a UNet with any skip connections
        Input : - mask : initial mask the partial data
                - kernel_size : int, size of the kernel for the partial convolution, except the last one
                - activation : string, activation function, as known by keras for Conv2D
    ...
    def __init__(self,mask,train, final, N_output = 1,input_shape = (288, 256, 10),SE_prob = 0.3, kernel_size=3,activation='relu',depth = 3, n_filter = 4, dropout_prob=0.2):
        super(UNet_transfer2,self).__init__()
        ...
        outputs1 = tf.keras.activations.linear(outputs1)
        #outputs1 = outputs1 * mask
        self.model = tf.keras.Model(inputs = inputs_img, outputs = outputs1)
        self.model.summary()
    def call(self, inputs_img):
...
class UNet_transfer3(tf.keras.Model):
    '''Build a UNet with any skip connections
        Input : - mask : initial mask the partial data
                - kernel_size : int, size of the kernel for the partial convolution, except the last one
                - activation : string, activation function, as known by keras for Conv2D
    ...
    def __init__(self,mask,train, final, N_output = 1,input_shape = (288, 256, 10),SE_prob = 0.3, kernel_size=3,activation='relu',depth = 3, n_filter = 4, dropout_prob=0.2):
        super(UNet_transfer3,self).__init__()
        ...
        outputs1 = min_sit + tf.keras.activations.relu(outputs1)
        #outputs1 = outputs1 * mask
        self.model = tf.keras.Model(inputs = inputs_img, outputs = outputs1)
        self.model.summary()
    def call(self, inputs_img):
        return self.model(inputs_img)
class UNet_transfer_concat(tf.keras.Model):
    ...
    def __init__(self,mask,train, final, N_output = 1,input_shape = (288, 256, 10),SE_prob = 0.3, kernel_size=3,activation='relu',depth = 3, n_filter = 4, dropout_prob=0.2):
        super(UNet_transfer_concat,self).__init__()
        ...
        outputs1 = tf.concat([inputs_img, outputs1], axis = 3)
        #alpha = tf.math.exp(min_sit)
        #beta = 1
        #def my_softplus(z):
         #   return 1/beta*tf.math.log(beta*tf.math.exp(tf.cast(z, dtype = tf.float64)) + alpha)
        ...
        outputs1 = PConv(filters = 8,
                    ...
                    use_bias=True,
                    kernel_initializer = tf.keras.initializers.Zeros())(outputs1)
        outputs1 = min_sit + PConv(filters = 1,
                    mask=mask2,
                    over_compensation = True,
                    kernel_size = 1,
                    padding="same",
                    activation = final,
                    use_bias=True)(outputs1)
