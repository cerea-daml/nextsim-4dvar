...
class PConv(layers.Conv2D):
    ...
    def build(self, input_shape):
        ...
        if self.oc :
            self.mask_ratio = window_size * self.mask / (valid_pixels+1)
        else :
...
class UNet3(tf.keras.Model):
    ...
    def __init__(self,mask,activation_final_layer = 'linear', input_shape = (288, 256, 10),N_output=2,over_compensation = True, kernel_size=3,SE_prob = 0.3,activation='relu',depth = 3, n_filter = 4, dropout_prob=0.2):
...
class ResNet(tf.keras.Model):
    ...
    def __init__(self, mask,input_shape,n_filter,activation_final_layer = 'linear',over_compensation = False,  N_output=2, dropout_prob=0.2, SE_prob = 0,depth = 0, kernel_size = 5, activation = 'relu',filters=[16,32,64],reg=1e-6,dropout=0.3):
        super(ResNet,self).__init__()
        F1,F2,F3 = filters
        mask = tf.expand_dims(mask, axis = 0)
        X_input = layers.Input(input_shape, name = 'inputs_img')

        X = PConv(filters = F3,mask=mask,over_compensation = 'False',kernel_size = 3,padding = 'same',
       ...
       # X_shortcut = PConv2D(filters = F3,
        ...
        X = PConv(filters = 8,kernel_size = 3,mask = mask,
                            ...
                            over_compensation = 'False',
                            kernel_initializer = tf.keras.initializers.HeNormal())(X)
        outputs = PConv(filters = N_output,kernel_size = 1,mask = mask, over_compensation = 'False',padding = 'same', activation = 'linear',kernel_initializer='zeros')(X)
